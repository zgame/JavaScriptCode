...
<!--      </el-table-column>-->
      ...
      <el-table-column label="name" width="420" align="center">
        <template slot-scope="scope">     <el-tag>{{scope.row.name}}</el-tag>        </template>
      </el-table-column>
      <el-table-column label="value" width="120" align="center">
        <template slot-scope="scope">     <el-tag>{{scope.row.value}}</el-tag>        </template>
      </el-table-column>
...
<!--      </el-table-column>-->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dlgData" :model="dlgData" label-position="left" label-width="120px" style='width: 400px; margin-left:50px;'>
        ...
        <el-form-item label="name" prop="server">
          <el-input v-model="dlgData.name"></el-input>
        </el-form-item>
        <el-form-item label="value" prop="server">
          <el-input v-model="dlgData.value"></el-input>
        </el-form-item>
...
<script>
  import { getList, actionAddList, actionEditList, actionDelList } from '@/api/portia_control'
  ...
  export default {
    ...
    methods: {
      ...
      addList(index, row) {
        ...
        this.dlgData.id = '1'
        this.dlgData.name = ''
        this.dlgData.value = '1'
      },
      AddListAction(index, row) {
        this.$refs.dlgData.validate((valid) => {
          if (valid) {
            return new Promise((resolve, reject) => {
              actionAddList(this.dlgData.id, this.dlgData.name, this.dlgData.value).then(response => {
      ...
      editList: function(index, row) {
        ...
        this.dlgData.id = '1'
        this.dlgData.name = '0'
        this.dlgData.value = '0'
  
        this.dlgData.id = row.id
        this.dlgData.name = '' + row.name
        this.dlgData.value = '' + row.value
      },
          ...
          if (valid) {
            return new Promise((resolve, reject) => {
              this.$confirm('确定修改么?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
                return new Promise((resolve, reject) => {
                  actionEditList(this.dlgData.id, this.dlgData.name, this.dlgData.value).then(response => {
